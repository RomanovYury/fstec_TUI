...
def compute_criticality(row, num_of_components):
    """Расчёт критичности для одной строки."""
    base = calc_cvss(row["Cvss Вектор"]) # Icvss 
    coeff = TYPE_COEFF.get(row["Тип компонента"], 0.1) * 0.5 # K * k(0.5)
    internet = ( INTERNET_BONUS if row["Доступ в интернет"] else 0.6 ) * 0.3 # P *p (0.3) 
...
def main():
    ...
    if args.score_col:
        ...
        ans = input("\nЕсть ли в файле колонка с вектором атаки уязвимости (например, CVSS_vect)? (yes/no): ").strip().lower()
        if ans == 'yes':
            score_col = choose_column(df, "Выберите колонку с вектором:")
            use_default_score = False
        else:
    ...
    merged["Критичность"] = merged.apply(lambda row: compute_criticality(row, num_of_components), axis=1)
